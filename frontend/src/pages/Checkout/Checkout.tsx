...
export default function Checkout() {
  ...
  const [{ message, status }, setData] = useState<{ status: "loading" | "success" | "failed"; message: string }>({
    status: "loading",
    ...
    const verifyPayment = async () => {
      ...
      try {
        const response = await axios
          .get(`${import.meta.env.VITE_API_URL}/stripe/confirm-payment?session_id=${sessionId}&token=${token}`)
          .then((res) => res.data);
        if (response.success) {
      ...
      <div className="bg-[rgb(40,40,40)] p-8 sm:p-10 gap-4 flex flex-col items-center rounded-2xl shadow-xl max-w-md sm:max-w-lg w-full text-center border border-gray-700">
        {status === "loading" ? (
          ...
          <div className="animate-fade-in">
            <div
              className={`text-6xl sm:text-7xl ${status === "success" ? "text-green-500 mb-4" : "text-red-500 mb-4"}`}
            >
              {status === "success" ? "✔️" : "❌"}
            </div>
            <h2 className={`text-xl sm:text-2xl font-bold ${status === "success" ? "text-green-400" : "text-red-400"}`}>
              {message}
            </h2>
        ...
        <Button
          className="bg-secondary-500 text-white py-2 px-6 rounded-lg mt-6 transition duration-300 
