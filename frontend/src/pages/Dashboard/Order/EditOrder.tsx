  ...
  return (
    ...
    >
      <div className="flex flex-col gap-6 py-4 w-full md:items-center ">
        <p className="font-bold text-lg md:text-xl font-anton text-sky-300">
          Products
        </p>
        ...
        <Track
          status={(step !== undefined && stages[step]) || status}
          updateState={(i: number) => setStep(i)}
        />
      </div>
      <div className="w-full sticky z-20 bottom-0 flex gap md:self-center mt-auto gap-2">
        <button
          onClick={onClose}
          className="p-2 flex-1 font-anton font-semibold bg-sky-600 "
        >
          Cancel
        </button>
        <button
          onClick={handleUpdate}
          className="p-2 flex-1 font-anton font-semibold bg-secondary-600 "
        >
          Update
        </button>
      </div>
