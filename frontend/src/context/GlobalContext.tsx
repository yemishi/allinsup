...
type Action =
  ...
  | {
      type: "INCREMENT_AMOUNT" | "DECREMENT_AMOUNT" | "UPDATE_PRODUCT_AMOUNT";
      payload: { product: CartType; amount: number };
    }
  | {
      type: "FIND_IN_CART";
      payload: { cart: CartType[]; product: ProductType | CartType };
    }
  | { type: "UPDATE_CART"; payload: CartType[] }
  | {
      type: "SET_PAYMENT_METHOD";
      payload: "Dinheiro" | "Cartão de Crédito" | "Cartão de Débito" | "Pix";
    }
  | { type: "RESET_CART" }
  | {
      type: "SET_BRAND_HOME";
  ...
  switch (action.type) {
    ...
    case "FIND_IN_CART":
      const foundCartItem = state.cart.find(
        (detail) =>
          detail._id === action.payload.product._id &&
          detail.updatedName === action.payload.product.updatedName
      );
    ...
    case "ADD_PRODUCT":
      const {
        ...
        category,
        updatedName,
      } = action.payload.toCart;

      if (!stock) return { ...state };

      const findProduct = state.cart.find(
        (detail) => detail._id === _id && detail.updatedName === updatedName
      );
      const updatedCartAdd = findProduct
        ? state.cart.map((e) =>
            e._id === _id &&
            e.updatedName === updatedName &&
            e.stock >= e.amount + 1
              ? { ...e, amount: e.amount + 1 }
              : e
          )
    ...
    case "INCREMENT_AMOUNT":
      ...
      const foundProduct = state.cart.find(
        (detail) =>
          detail._id === product._id &&
          detail.updatedName === product.updatedName
      );
      ...
      if (!foundProduct) {
        ...
        const updatedCartInc = state.cart.map((e) =>
          e.updatedName === product.updatedName &&
          e._id === product._id &&
          e.amount + amount <= e.stock
    ...
    case "DECREMENT_AMOUNT":
      const { product: decrementProduct, amount: decrementAmount } =
        action.payload;
      const updatedCartDec = state.cart.map((e) =>
        e.updatedName === decrementProduct.updatedName &&
        e._id === decrementProduct._id &&
        e.amount - decrementAmount >= 1
    ...
    case "UPDATE_PRODUCT_AMOUNT":
      ...
      const productInCart = state.cart.find(
        (e) =>
          e.updatedName === updateProduct.updatedName &&
          e._id === updateProduct._id
      );
...
const GlobalContextProvider: React.FC<GlobalContextProviderProps> = ({
  ...
  useEffect(() => {
    document.body.style.overflow =
      state.cartOpen || state.userOpen || state.burgerOpen || state.addressOpen
        ? "hidden"
        : "auto";
  ...
  useEffect(() => {
    ...
    if (savedCart) {
      ...
      const fetchDataForProduct = async (product: CartType) => {
        ...
        try {
          const response = await axiosRequest.productInfo(
            flavor,
            sizeProduct,
            _id
          );
          const variantSelected = response.data.variants.find(
            (variant) => variant.isSelected === true
          );
          const sizeSelected = variantSelected?.sizeDetails.find(
          ...
          toast.warn(
            "Não foi possivel recuperar alguns items do seu carrinho."
          );
          console.log(error);
          return null;
        }
      ...
      const updateCart = async () => {
        const updatedProductsPromises = savedProducts.map((product) =>
        ...
        if (isChanged) {
          toast.warn(
            "Houve alterações em alguns produtos em seu carrinho. Já atualizamos para você."
          );
        }

        dispatch({
          type: "UPDATE_CART",
          payload: updatedProducts.filter(
            (product) => product !== null
          ) as CartType[],
        });
        localStorage.setItem(
          "cart",
          JSON.stringify(updatedProducts.filter((product) => product !== null))
        );
      };
