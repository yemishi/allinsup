import { ChangeEvent, ReactEventHandler, useState, lazy } from "react";
...
import { Slide, Slider, SliderProps, alert } from "../../../components";
...
export default function ProductInfo({ product, similarProducts }: { product: ProductType, similarProducts: ProductType[] | undefined }) {
    const { cart, incrementAmount: updateAmount, } = useGlobalState()
    const [variantIndex, setVariantIndex] = useState<number>(0)
    const [amount, setAmount] = useState<number>(1)
    const [sizeIndex, setSizeIndex] = useState<number>(0)
    const { _id, sizeDetails, variants, flavor, name, photos, price, sizeProduct, desc, amount: amountInCart, stock, updatedName, promotion } = productDetails(product, cart, variantIndex, sizeIndex)
    ...
    const decrementAmount = () => {
        if ((amount - 1)) setAmount((value) => value - 1)
        else setAmount(1)
    }

    const addInCart = () => {
        if (amountInCart && (amount + amountInCart) >= stock) return alert("Não foi possível adicionar o produto. A quantidade pedida não está disponível.", { variant: "error" })
        else {
            alert("Produto adicionado ao carrinho", { variant: "success" })
            updateAmount(product.toCart, amount)
        }
    }
    ...
    return <div key={`${_id} ${updatedName}`} className="text-white w-full rounded-lg  gap-6 flex flex-col">
        ...
        <div className="flex flex-col gap-6 pb-6 w-full p-4 bg-primary-500 rounded-md">
            ...
            <div className="flex justify-center items-center">
                <button className="bg-sky-400 w-full p-2 rounded font-anton font-semibold">Comprar agora</button>
            </div>
        </div>
