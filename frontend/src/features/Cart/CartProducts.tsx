...
export default function CartProducts() {
  ...
  const handleChange = (value: string, stock: number, productIndex: number) => {
    const updatedValue =
      Number(value) > stock ? stock : Number(value) < 1 ? 1 : Number(value);
  ...
  return (
    <div className="flex flex-col p-3 font-anton">
      {cart.map((product, index) => {
        const { name, coverPhoto, price, promotion, amount, _id, stock } =
          product;
        return (
          <div
          ...
          >
            ...
            <div className="flex flex-col text-sm lg:text-lg w-full">
              <span className="hover:text-secondary-500 duration-300">
                {name}
              </span>

              <button
                onClick={() => remove(index)}
                className="absolute top-2 right-0 cursor-pointer self-start"
              >
                <IoIosClose className="h-7 w-7 md:w-9 md:h-9" />
              </button>
              ...
              <div className="flex justify-between mt-auto items-end">
                <p className="text-secondary-500 font-bold text-lg lg:text-xl">
                  {parseLocalCurrency(
                    promotion ? Number(promotion) : Number(price)
                  )}
                </p>
