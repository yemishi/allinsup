...
export default function CartProducts() {
    const { cart, removeProduct, incrementAmount, decrementAmount, setCartOpen, updateProductAmount } = useGlobalState()
            ...
            {cart && cart.map((product) => {
                ...
                return <div key={`${product._id}_${name}_${price}`} className="!flex flex-row gap-3 h-[170px] border-b border-zinc-700 py-6 pb-5 relative" >
                    <Link className="!flex " to={`/${urlReplace(category)}/${urlReplace(`${name}-${flavor}-${sizeProduct}`)}/${_id}`}>
                        <div onClick={() => setCartOpen(false)} className="w-auto flex bg-white p-2 rounded-md">
                    ...
                    <div className="flex flex-col text-sm  w-full">
                        <div className="flex gap-3">
                            ...
                            <button onClick={() => removeProduct(product)} className="cursor-pointer self-start">
                        ...
                        <div className="flex justify-between mt-auto items-end ">
                            ...
                            <span className="flex border-gray-600 p-2 rounded-lg gap-2 border">
                                ...
                                <button onClick={() => decrementAmount(product, 1)} className="w-5">
                                ...
                                <button onClick={() => incrementAmount(product, 1)} className="w-5">
                    ...
                    <div className="ml-auto flex flex-col absolute top-0 right-0 h-full">
