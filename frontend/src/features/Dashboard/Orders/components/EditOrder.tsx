...
export default function EditOrder({ orderId, closeParent, directionDrag, setDirectionDrag, setState, state, }: PropsType) {
    ...
    if (error || !data) return <ErrorPage msg="Algo deu errado"/>
    ...
    return (
        <motion.div onClick={closeParent} animate={state ? "open" : "exit"} variants={variantsParent} className="flex  md:justify-center flex-col w-full h-full fixed top-0 backdrop-brightness-50">
            <DivDraggable directionDrag={directionDrag} initialDirection="-100%" setDirectionDrag={setDirectionDrag} state={state} setState={setState}
                classAddition="md:w-auto md:h-auto md:self-center justify-self-center md:border-2 md:border-primary-200 md:rounded-lg" closeParent={closeParent}>

                <div className="flex flex-col gap-6 py-4 w-full   md:items-center">

                    <p className="font-bold text-lg md:text-xl font-anton text-sky-300">Produtos</p>
                    
                    <div className="flex md:grid md:gap-3">

                        {products.map((product, index) => {
                            const { coverPhoto, name, productQtd, productPrice } = product

                            return <div key={`${product}_${index}`} className="flex md:p-4 md:border-t md:border-primary-200 relative bg-primary-500 w-full font-anton gap-4">
                    ...
                    <p className="font-bold text-lg md:text-xl font-anton text-sky-300">Estado da encomenda</p>
                ...
                <span className="w-full sticky z-30 bottom-0 flex gap md:self-center">
