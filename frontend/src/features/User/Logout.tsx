...
export default function Logout({ tel, setIsAuth }: { tel: string, setIsAuth: Dispatch<React.SetStateAction<boolean>> }) {
    ...
    return (
        <motion.div onClick={() => dispatch({ type: "SET_USER_OPEN", payload: false })} className="backdrop-brightness-50 h-full w-full items-start
         flex left-0 top-0 absolute z-30">
            <motion.div onClick={(e) => e.stopPropagation()} initial={{ y: "-100%" }} animate={{ y: 0 }} className="text-white flex flex-col
