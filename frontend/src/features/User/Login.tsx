...
export default function Login() {
    ...
    const inputStyle = `block py-2.5 ps-6 pe-0  text-sm md:text-base text-white bg-transparent border-0 border-b-2  border-gray-300 appearance-none
     dark:focus:border-secondary-500 focus:outline-none focus:ring-0 focus:border-secondary-600 peer`

    const lableStyle = `absolute text-sm md:text-base text-white text-opacity-50 duration-300 transform -translate-y-6 scale-75 top-3 pointer-events-none origin-[0] 
    ...
    return (<motion.div onClick={handleClose} animate={isExiting ? { opacity: 1 } : { opacity: 0, transition: { delay: 0.3 } }} className="w-full z-30 h-screen
     overflow-hidden fixed top-0 backdrop-brightness-50 backdrop-blur-sm" >
                ...
                <div className=" w-full bg-primary-500 bg-opacity-90 flex flex-col gap-7 py-4 shadow-md shadow-primary-500">
                    ...
                    <form onSubmit={submitLogin} className="max-w-xs mx-auto flex flex-col gap-8 w-full items-center">
                        ...
                        <div className="relative">
                            ...
                            <input value={phoneNumber} onChange={handlePhoneInputChange} type="text" className={inputStyle} pattern={phoneNumberPattern} placeholder=" " required title="O formato deve ser (XX) X XXXX-XXXX" />

                            <label className={lableStyle}>Insira seu número aqui</label>
        ...
        <div onClick={(e) => e.stopPropagation()} className="hidden pt-6 items-center bg-primary-550 w-80 md:flex flex-col gap-7 z-40">
            <h2 className="font-anton text-xl text-center font-bold md:text-2xl text-secondary-100">Iniciar Sessão</h2>
            ...
            <form onSubmit={submitLogin} className="max-w-xs mx-auto flex flex-col gap-8 w-full items-center">
                ...
                <button type="submit" className="bg-secondary-700 w-full mt-auto font-anton p-1 font-bold text-base rounded-md">Confirmar</button>
            </form>
        </div>
    </motion.div >
    )
