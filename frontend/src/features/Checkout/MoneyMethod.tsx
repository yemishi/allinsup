...
export default function MoneyMethod({ setMoneyState, setPurchaseOpen }: {
    ...
    const labelClass = `${!checkValue && change ? "text-red-500 peer-focus:text-red-500" : "text-white peer-focus:text-secondary-500"} absolute text-sm text-opacity-50 duration-300 transform -translate-y-7 right-0 scale-75 top-3 pointer-events-none  
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} onClick={() => setMoneyState(false)} className="w-full h-full fixed backdrop-brightness-50 z-20 flex justify-center items-center">
            <div onClick={(e) => e.stopPropagation()} className="bg-primary text-gray-200 w-11/12 h-3/6 rounded-lg text-center gap-2 flex flex-col p-5">

                <h2 className="text-xl font-anton font-semibold px-3">Você escolheu pagar em dinheiro</h2>

                <div className="font-anton font-semibold text-gray-300 text-left flex flex-col mt-6">
                ...
                <div className="relative w-3/6 self-center mt-4">
