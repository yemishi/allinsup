...
import { totalPrice, parseLocalCurrency, reloadPage, blinkVariant } from "../../utils"
...
export default function PurchaseSummary() {
    ...
    return (
        <motion.div variants={blinkVariant} initial="initial" animate="animate" exit="exit" transition={{ duration: 0.2 }} className="w-full min-h-full items-center py-8 flex flex-col fixed backdrop-brightness-50 text-white bg-primary z-20 pb-0">
            ...
            <div className="mt-10 bg-primary-500 lg:bg-transparent w-full lg:w-[80%] p-6 flex flex-col justify-center gap-4">

                {isPix && <div className="flex flex-col gap-2 md:self-center md:w-3/6">
                    <p className="font-lato font-semibold text-secondary-200 text-center md:text-lg lg:text-xl">Chave Pix para pagamento</p>
                    <span className="flex relative gap-2 font-anton md:text-lg font-bold text-base p-3 border-2 rounded-lg border-secondary-700 bg-primary-600">
                        ...
                        <button onClick={copyToClipboard} className="ml-auto font-mono text-sm md:text-base lg:text-lg absolute right-0 top-0 h-full px-4 
                         rounded-r-md bg-secondary-700 ">Copiar</button>
                    </span>
                </div>}

                <div className="flex flex-col gap-2  md:w-3/6 md:self-center md:text-lg lg:text-xl">
                    ...
                    <a href={`https://api.whatsapp.com/send?phone=${import.meta.env.VITE_PHONE_NUMBER}&text=${msg}`} className="flex gap-2 font-mono
                         items-end font-semibold text-base p-3 border-2  rounded-lg border-green-600 text-green-500 md:text-lg lg:text-xl md:items-center
            ...
            <div className="grid grid-cols-2 mt-auto w-full font-anton font-bold md:text-lg lg:text-xl text-gray-200 text-center ">
